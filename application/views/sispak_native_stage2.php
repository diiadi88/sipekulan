<html>
	...
	<body style="padding: 10px">
		...
		<?php
			...
			$acuan=$_POST["acuan"]; //contoh isian: 1,7,3,
			$conn = new mysqli($servername, $username, $password, $dbname);
			$query = "SELECT distinct(if(trim(coalesce(substr(path,$pfilter+1,locate(',',substr(path,$pfilter+1,length(path)-$pfilter))-1),''))='','-1',substr(path,$pfilter+1,locate(',',substr(path,$pfilter+1,length(path)-$pfilter))-1))) as pilih FROM $table where path like '%$acuan,%' and substr(path,1,$pfilter)='$filter'";
			echo "SELECT distinct(if(trim(coalesce(substr(path,$pfilter+1,locate(',',substr(path,$pfilter+1,length(path)-$pfilter))-1),''))='','-1',substr(path,$pfilter+1,locate(',',substr(path,$pfilter+1,length(path)-$pfilter))-1))) as pilih FROM $table where path like '%$acuan,%' and substr(path,1,$pfilter)='$filter'";
			...
			else
			{
				...
				$hasil.="<input type='hidden' name='acuan' value='$acuan'>";					
